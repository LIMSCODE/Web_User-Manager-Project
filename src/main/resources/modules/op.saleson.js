...
$s.api = {
     ...
     * @JsonPath password
     */
    getAuthToken: function (req, successHandler, failureHandler) {      // 로그인창에서 호출, req에 id, pw포함됨
        if (req == null) {
            return;
        }
        ...
        $s.axios
            .post(`http://localhost:8080/api/user/login`, req)       //req = vm.loginRequest 로 넘김 ,,
                                                                        //form에 user.xx 붙여서 넘김,,user로 받음
                //{        //id, pw포함된 req값을 컨트롤러로 보냄
                // headers: {
                //     Hmac: hashInBase64
                // }})
            .then(function (response) {                                 //컨트롤러로 보낸 결과 (토큰) 를 받음
                ...
                // $s.setToken(response.data.token, response.status, 'USER');      //세션에 토큰저장 $s.core.setSession($s.const.TOKEN, token);
...
$s.core = {
    ...
    getSession: function (key) {        //사용되는부분
